...
def find_amplitude(signal):
    ...
    amplitudes = 2 / 50002* np.abs(np_fft) 
...
if 'frequency_sample' not in st.session_state:
    st.session_state['frequency_sample'] =4
...
if 'fig' not in st.session_state:
    st.session_state['fig'] = px.line(x=t, y=np.zeros(len(t))).update_layout(xaxis_title="Time (Sec)", yaxis_title="Amplitude")
...
def download(time , magnitude):
    ...
    add_number_y=[1]
    worksheet.write_column(50001,1,add_number_x)
    worksheet.write_column(50001,0,add_number_y)
...
if upload_file:
    ...
    if frequency==0:
        ...
        if noise_ck:
            ...
            new_signal = Noise(st.session_state['signal'], number,50001)
        ...
        if frequency_sample!=0:
            ...
            n_Sample=np.arange(0,1+1/T,1/T)
            t_sample = n_Sample * T
    ...
    if st.sidebar.button("Add Signal"):
        ...
        st.session_state['fig'] = px.line(x=t, y=st.session_state['signal']).update_layout(xaxis_title="Time (Sec)", yaxis_title="Amplitude")
    ...
    if sampleByfreq_ck:
        sampleByFreq_sl = st.sidebar.slider("Frequency", min_value=1,value=st.session_state['sample_by_frequency'])
        st.session_state['frequency_sample']=sampleByFreq_sl
        st.session_state['sample_by_frequency']= sampleByFreq_sl
    else:
        sampleRateUp = st.sidebar.slider("Sample rate", min_value=0,max_value=10,value=st.session_state['sample_rate'])
        st.session_state['frequency_sample']= sampleRateUp*frequency
    ...
    if noise:
        ...
        st.session_state['signal'] += new_signal
    # elif noise==False:
    #     st.session_state['signal'] -= new_signal
        
    
    st.session_state['fig'] = px.line(x=t, y=st.session_state['signal']).update_layout(xaxis_title="Time (Sec)", yaxis_title="Amplitude")
    st.session_state['fig'].update_yaxes(title_font=dict(size=18,family="Arial"))
    st.session_state['fig'].update_xaxes(title_font=dict(size=18,family="Arial"))
    
    
    #sampling func
    if st.session_state['frequency_sample']!=0:
        T=1/st.session_state['frequency_sample']
        ...
        st.session_state['fig'].add_scatter(x=t_sample, y=signal_sample, mode="markers",name="samples points", marker={"color":"black"})
        ...
        if Inter:
            ...
            for i in n_Sample:
                ...
                sum+= np.dot(s_sample,np.sinc((t-i*T)/T))
            signal_intr=sum
            st.session_state['fig'].add_scatter(x=t,y=signal_intr, mode="lines",name="Reconstructed signal", line={"color":"red"})
